...
# Per-account launch-option status report shape
LaunchStatus = dict[str, str]

# Window-hiding kwargs for subprocess calls (Windows only)
_SUBPROCESS_HIDE = {}
if base.is_win:
    _SUBPROCESS_HIDE["creationflags"] = 0x08000000  # CREATE_NO_WINDOW
...
def restore_boot_language() -> bool:
    """
    ...
    boot_vcfg_path = os.path.join(
        config.get("steam_library"), "steamapps", "common", "dota 2 beta", "game", "dota", "cfg", "boot.vcfg"
    )
...
def get_steam_root_path():
    ...
    MacOS: default
    """
    if config.get("debug_simulate_no_steam", False):
        config.set("steam_root", "")
        config.set("steam_library", "")
        return ""
