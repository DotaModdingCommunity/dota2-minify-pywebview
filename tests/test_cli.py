...
def test_patch_accepts_config_and_mods_paths(tmp_path):
    ...
    mods_path = str(tmp_path / "mods.json")
    with (
        ...
        result = runner.invoke(app, ["patch", "-c", config_path, "-m", mods_path])

        assert result.exit_code == 0
        assert base.main_config_file_dir == config_path
        assert base.mods_config_dir == mods_path
...
def test_mods_show_outputs_valid_json():
    ...
    with (
        ...
        mock_patch("cli._config.write_json_file"),
    ):
...
def test_ensure_mods_file_defaults_always_on_mod_to_true(tmp_path, monkeypatch):
    import json as json_mod

    import cli

    mods_file = str(tmp_path / "mods.json")
    monkeypatch.setattr(base, "mods_config_dir", mods_file)
    monkeypatch.setattr(constants, "mods_with_order", ["#base", "Normal Mod"])
    monkeypatch.setattr(mods_shared, "get_always_on_mods", lambda: {"#base"})
    monkeypatch.setattr(mods_shared, "scan_mods", lambda: None)

    cli._ensure_mods_file()
...
def test_ensure_mods_file_rewrites_stale_false_for_always_on_mod(tmp_path, monkeypatch):
    import json as json_mod

    import cli

    mods_file = str(tmp_path / "mods.json")
    with open(mods_file, "w") as file:
        file.write('{"#base": false, "Normal Mod": false}')
    monkeypatch.setattr(base, "mods_config_dir", mods_file)
    monkeypatch.setattr(constants, "mods_with_order", ["#base", "Normal Mod"])
    monkeypatch.setattr(mods_shared, "get_always_on_mods", lambda: {"#base"})
    monkeypatch.setattr(mods_shared, "scan_mods", lambda: None)
