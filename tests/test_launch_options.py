...
def test_inserts_before_command_macos(monkeypatch, mock_frozen_env):
    import vdf
    from core import base
    ...
    assert (
        vdf_data["UserLocalConfigStore"]["Software"]["Valve"]["Steam"]["apps"][base.STEAM_DOTA_ID]["LaunchOptions"]
        == "/bin/zsh -c '/path/to/minify prelaunch' && %command% -novid -language english"
    )


def test_posix_prefix_quotes_path_with_spaces(monkeypatch, mock_frozen_env):
    import vdf
    from core import base

    monkeypatch.setattr(base, "is_win", False)
    monkeypatch.setattr(base, "is_linux", True)
    monkeypatch.setattr("sys.executable", "/home/john doe/Applications/Minify/Minify")
    ...
    assert (
        vdf_data["UserLocalConfigStore"]["Software"]["Valve"]["Steam"]["apps"][base.STEAM_DOTA_ID]["LaunchOptions"]
        == "bash -c '/home/john doe/Applications/Minify/Minify prelaunch' && %command% -novid"
    )
...
def test_conditional_patch_applies_to_all_accounts(monkeypatch):
    from core import base
